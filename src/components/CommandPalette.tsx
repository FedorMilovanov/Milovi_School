...
export default function CommandPalette({
  ...
  const quickActions: QuickAction[] = useMemo(() => {
    ...
    return [
      ...
      ...chefCats.map(c => ({
        id: `nav-${c.id}`, label: c.name, sublabel: c.description, icon: c.icon,
  ...
  const handleKeyDown = useCallback((e: ReactKeyboardEvent) => {
    ...
    if      (e.key === 'ArrowDown') { e.preventDefault(); setActiveIndex(i => (i + 1) % totalItems) }
      ...
      if (showQuickActions && activeIndex < quickActions.length) {
        ...
        const idx    = showQuickActions ? activeIndex - quickActions.length : activeIndex
        const result = articleResults[idx]
        if (result) { onClose(); onOpenArticle(result.item) }
      }
    }
                    ...
                    <div>
                      ...
                      <div className="pb-1">
                        {quickActions.map((action, idx) => {
                          ...
                          return (
                            <button
                              key={action.id} type="button" data-item
                            ...
                            >
                              ...
                              <div className="min-w-0 flex-1">
                                ...
                                {action.sublabel && (
                                  <p className="truncate font-mono text-[10px]" style={{ color: 'var(--text-muted)' }}>{action.sublabel}</p>
                                )}
                              </div>
                              <ArrowRight />
                            </button>
                          ...
                          >
                            ...
                            <div style={{ position: 'relative', marginTop: 2, flexShrink: 0 }}>
                              ...
                              {(result.pct ?? 0) > 0 && (
                                <div style={{
                                  ...
                                  background: 'var(--text-accent)', borderRadius: '0 0 0 2px',
                                }} />
                              )}
                              ...
                              {isActive && article.image && (
                                <motion.div layoutId="cp-active-ring"
                                  style={{
                                    ...
                                    border: '1.5px solid var(--text-accent)', borderRadius: 5,
                            ...
                            <div style={{ minWidth: 0, flex: 1 }}>
                              ...
                              <p className="mt-0.5 truncate font-mono text-[10px]" style={{ color: 'var(--cp-text-mid)' }}>
                                ...
                                {cat?.name ?? article.category}{' · '}{article.readTime} мин
                                {article.author && (
                                  <>{' · '}<span style={byAuthor ? { color: 'var(--text-accent)', opacity: 0.8 } : undefined}>
                                    <H text={article.author} matches={result.matches} field="author" />
                                  </span></>
                                )}
                              </p>
                              <AnimatePresence>
