...
function Icon({ id, active }: { id: string; active: boolean }) {
  ...
  return <svg className={cls} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}><path strokeLinecap="round" strokeLinejoin="round" d="M21 21l-5.197-5.197m0 0A7.5 7.5 0 105.196 5.196a7.5 7.5 0 0010.637 10.637z" /></svg>
}
          ...
          <div className="relative overflow-hidden rounded-[16px] border border-amber-700/20 bg-[var(--bg-overlay-95)] shadow-xl shadow-black/10 backdrop-blur-xl" style={{ paddingBottom: 'env(safe-area-inset-bottom, 0px)' }}>
            ...
            <div className="grid grid-cols-4 items-center justify-items-center py-2">
              {tabs.map((tab) => {
                ...
                return (
                  <button key={tab.id} type="button" onClick={() => actionById[tab.id]()} className="relative flex w-full flex-col items-center gap-1 py-1 text-center transition active:scale-95" aria-current={active ? 'page' : undefined}>
                    {active && <motion.span layoutId="mobile-active-pill" className="absolute inset-x-2 inset-y-0 rounded-[12px] bg-stone-950/5 dark:bg-amber-100/10" />}
